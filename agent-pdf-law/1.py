...
# ===================== 固定配置（适配Ollama）=====================
# 若出现 "unable to allocate CPU buffer" 说明内存不足：改用更小模型、关闭其他程序，或先只跑嵌入再问答
OLLAMA_MODEL = "qwen2.5:0.5b"  # 极省内存；若内存充足可改为 qwen2.5:1.5b / 3b / 7b
OLLAMA_BASE_URL = "http://localhost:11434"  # Ollama 默认本地地址
EMBEDDING_MODEL = "bge-m3"  # 嵌入模型（需：ollama pull bge-m3）；与对话模型会同时占内存
...
# ===================== 前端展示（Streamlit）=====================
def run_web_ui():
    """Streamlit 前端：展示回答与检索到的文本内容"""
    import streamlit as st

    st.set_page_config(page_title="法务 RAG 智能助手", page_icon="⚖️", layout="wide")
    st.title("⚖️ 法务 RAG 智能助手")
    st.caption(f"基于 Ollama/{OLLAMA_MODEL} · 知识库检索增强")

    @st.cache_resource(show_spinner="正在加载法务知识库与模型…")
    def get_legal_qa():
        return build_legal_rag_chain()

    legal_qa = get_legal_qa()

    question = st.text_area("请输入你的法律问题", placeholder="例如：集成电路产业和软件产业是什么产业的核心？", height=100)
    col1, _ = st.columns([1, 4])
    with col1:
        submitted = st.button("提交提问")

    if submitted and question.strip():
        with st.spinner("正在检索知识库并生成回答…"):
            try:
                answer, context_docs = legal_qa(question.strip())
                st.success("回答生成完成")
                st.subheader("📋 法务助手回答")
                st.markdown(answer)
                ...
                for i, doc in enumerate(context_docs, 1):
                    ...
                    source_name = source.get("source", "未知文档")
                    if isinstance(source_name, str):
                        source_name = os.path.basename(source_name)
                    page = source.get("page", "")
                    title = f"片段 {i}：{source_name}"
                    if page != "":
                ...
                err_msg = str(e)
                st.error(f"回答生成失败：{err_msg}")
                if "allocate" in err_msg or "buffer" in err_msg or "500" in err_msg:
                    st.info("💡 **内存不足**：请在 1.py 顶部将 `OLLAMA_MODEL` 改为更小模型（如 `qwen2.5:0.5b`），关闭其他程序，或先执行 `ollama stop` 再重试。")
    elif submitted and not question.strip():
        st.warning("请输入问题后再提交。")

    st.divider()
    st.caption("输入问题后点击「提交提问」即可查看回答与检索依据。关闭页面或刷新可清空当前会话。")
    st.caption("💡 启动本页面请在项目目录运行：`streamlit run 1.py`；命令行问答请运行：`python 1.py --cli`")
...
if __name__ == "__main__":
    ...
    if "--cli" in sys.argv:
        ...
        # 前端展示：若由 streamlit 启动则直接渲染；若直接 python 1.py 则自动调起 streamlit
        if "streamlit" in sys.modules:
            run_web_ui()
        else:
            print("正在启动前端页面… 请在浏览器中打开。")
            import subprocess
